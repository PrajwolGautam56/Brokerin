...
export const contactService = {
  ...
  submitContact: async (formData) => {
    try {
      ...
      // Use extended timeout for contact form submissions
      const response = await api.post('/api/contacts', {
        ...
        timeout: 90000 // 90 seconds for contact form submissions
      });
  ...
  updateContactInquiry: async (id, updateData) => {
    try {
      ...
      logger.error('Error updating contact inquiry:', error);
  ...
  deleteContactInquiry: async (id) => {
    try {
      ...
      logger.error('Error deleting contact inquiry:', error);
