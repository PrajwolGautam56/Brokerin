...
function UserDashboard() {
  ...
  const fetchFurnitureRequests = async () => {
    try {
      ...
      logger.error('Error fetching furniture requests:', err);
  ...
  const fetchPendingOverdue = async () => {
    try {
      ...
      logger.error('Error fetching pending/overdue payments:', err);
    }
  };
  ...
  const fetchDashboard = async () => {
    ...
    setError(null);
    try {
      logger.log('Fetching user dashboard...');
      const response = await userService.getMyDashboard();
      logger.log('Dashboard response:', response);
      ...
      if (response.success && response.data) {
        ...
        logger.log('Extracted data from response.data:', dashboardData);
      } else if (response.data) {
        // Handle if data is already extracted
        dashboardData = response.data;
        logger.log('Using response.data:', dashboardData);
      } else if (response.user || response.stats) {
        // Handle if response is already the data object
        dashboardData = response;
        logger.log('Using response as dashboard data:', dashboardData);
      }
      
      logger.log('Setting dashboard data:', dashboardData);
      
      // If still no data, create a fallback
      if (!dashboardData || (!dashboardData.stats && !dashboardData.user)) {
        logger.log('No proper dashboard data, using fallback structure');
        dashboardData = {
          user: 'User',
          stats: {
