...
function PropertyDetails() {
  ...
  const fetchProperty = async () => {
    try {
      ...
      logger.log('Property fetched:', response);
      logger.log('Property ID:', response._id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      logger.log('Property object:', property);
      logger.log('Property ID field:', property.property_id);
      logger.log('Property _id field:', property._id);
      logger.log('Using property_id:', propertyId);
      logger.log('Submitting property request:', requestData);
          ...
          <div className="p-6">
            ...
            <div className="py-6 border-t border-gray-200">
              ...
              <div className="flex items-center justify-between text-sm text-gray-500 mb-3">
                {(() => {
                  const lat = property.location_coordinates?.latitude ? parseFloat(property.location_coordinates.latitude) : null;
                  const lng = property.location_coordinates?.longitude ? parseFloat(property.location_coordinates.longitude) : null;
                  ...
                  return (
                  <span>Coordinates not provided by owner. Map shows an approximate location.</span>
                  );
                })()}
                <span>{getFullAddress()}</span>
              </div>
